...
export const AccordionUI: FC<AccordionCheckboxProps> = ({
  ...
  return (
    ...
    <div className = {styles.accordionPanel}>
      <CheckBoxUI aria-expanded={isActive} label={label} variant={'accordion'} isActive ={isActive} groupName={label} onChange={onChange} id={'_'}/>
      <IconButtonUI onClick={handleSetItems} isActive={isOpened} icon={closed} iconActive={opened}/>
    </div>
      <ul className={clsx( styles.accordionContent, isOpened && styles.accordionContentOpen)} aria-hidden={!isOpened}>
        {items.map((item) => (
