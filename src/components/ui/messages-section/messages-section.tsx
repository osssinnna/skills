...
export const MessagesSectionUI = forwardRef<
  HTMLElement,
  TMessagesSectionUIProps
>(({ unreadMessages, readMessages, isVisible }, ref) => {
  ...
  return (
    <section
      ...
      aria-label={"Уведомления о предложениях"}
      ref={ref}
      className={clsx(
        styles.messagesListSection,
        isVisible ? styles.messagesListSectionOpen : ""
      )}
    >
      {hasMessages ? (
        <>
          ...
          {readMessages.length > 0 ? (
            ...
            >{`Просмотренных ${emptyMessage}`}</p>
          )}
        </>
      ) : (
        <p className={styles.emptyState} role="status">
          {emptyMessage[0].toUpperCase() + emptyMessage.slice(1)}
        </p>
      )}
    </section>
  );
